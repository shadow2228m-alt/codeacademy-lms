...
import { upsertStudentAnswer, submitAndGradeQuiz, getMyStatsAction } from '@/app/student/quizzes/actions'
...
type XPEvent = { xpBefore: number; xpAfter: number; score: number; passed: boolean }
...
export default function ExamArena({ quiz, questions, attempt }: {
  ...
  const endAt = useMemo(
    () => new Date(new Date(attempt.started_at).getTime() + quiz.duration_minutes * 60 * 1000).getTime(),
    [attempt.started_at, quiz.duration_minutes]
  )
  ...
  function queueSave(qid: string, payload: any) {
    ...
    timers.current[qid] = setTimeout(async () => {
      ...
      } catch { /* يُحفظ في localStorage للمزامنة لاحقاً */ }
      finally { setSaving(null) }
    }, 500)
  }

  // استعادة الإجابات من localStorage عند الرجوع بعد انقطاع الشبكة
  useEffect(() => {
    const recovered: any = {}
    questions.forEach(q => {
  ...
  const autoSubmit = () => {
    ...
    startTransition(async () => {
      // احفظ XP قبل التسليم
      const statsBefore = await getMyStatsAction()
      const r = await submitAndGradeQuiz(attempt.id)
      setResult(r)
      // احسب XP بعد التسليم
      const statsAfter = await getMyStatsAction()
      if (statsBefore !== null && statsAfter !== null) {
        setXpEvent({
          xpBefore: statsBefore.computed_xp,
          xpAfter: statsAfter.computed_xp,
          score: r.score_achieved,
          passed: r.score_achieved >= quiz.total_score * 0.5
        })
      }
    })
  }

  const qTypeLabel = (t: string) => t === 'multiple_choice' ? 'اختيار من متعدد' : 'كود برمجي'
    ...
    <div dir="rtl" className="grid grid-cols-12 gap-5">
      ...
      <div className="col-span-12 flex items-center justify-between">
        ...
        <motion.div
          ...
          className={`px-5 py-2 rounded-2xl font-mono text-lg border transition-all ${
            danger
              ? 'border-red-500 text-red-400 shadow-[0_0_25px_rgba(239,68,68,0.35)]'
              : 'border-cyan-400/40 text-cyan-300 shadow-[0_0_18px_rgba(0,255,255,0.18)]'
          }`}
          style={{ direction: 'ltr' }}
        >
          ⏱ {mm}:{ss}
      ...
      <div className="col-span-12 space-y-6">
        {questions.map((q, idx) => (
          <div
            key={q.id}
            className={`rounded-2xl p-5 border ${danger ? 'border-red-500/40' : 'border-zinc-800'} bg-zinc-950/70`}
          >
            <div className="flex items-center justify-between mb-3">
              <div className="text-zinc-300">
                سؤال {idx + 1} •{' '}
                <span className="text-zinc-400">{q.max_points} نقطة</span> •{' '}
                <span className={q.q_type === 'multiple_choice' ? 'text-cyan-300' : 'text-fuchsia-300'}>
                  {qTypeLabel(q.q_type)}
                </span>
              </div>
              <div className="text-xs text-zinc-500">
                {saving === q.id ? 'يحفظ…' : '✓ حفظ تلقائي'}
              </div>
            </div>
            ...
            {q.q_type === 'multiple_choice' ? (
                    ...
                    <label
                      ...
                      className={`cursor-pointer px-4 py-3 rounded-xl border transition ${
                        checked
                          ...
                          : 'border-zinc-800 hover:border-zinc-600 bg-black/30'
                      }`}
                    >
                      <input
                        type="radio"
              ...
              <div>
                <CodeSandbox
                  ...
                  onRun={(code) => {
                    setTerminal('>>> تشغيل…\n' + code.slice(0, 400) + (code.length > 400 ? '...\n' : '\n') + '✓ لا أخطاء نحوية (محاكاة)')
                  }}
                />
                <div className="mt-3 text-[12px] bg-black/40 border border-zinc-800 rounded-xl p-3 text-emerald-300 font-mono whitespace-pre-wrap" dir="ltr">
                  {terminal}
                </div>
              </div>
            )}
        ...
        {!result ? (
          <button
            onClick={autoSubmit}
            disabled={isPending}
            className="w-full py-4 rounded-2xl bg-gradient-to-r from-emerald-400 to-cyan-400 text-black font-black text-lg disabled:opacity-50 hover:opacity-90 transition"
          >
          ...
          >
            ...
            <div className="mt-2 text-3xl font-black">{result.score_achieved} / {quiz.total_score}</div>
            <div className={`text-sm mt-1 font-bold ${result.score_achieved >= quiz.total_score * 0.5 ? 'text-emerald-400' : 'text-red-400'}`}>
              {result.score_achieved >= quiz.total_score * 0.5 ? 'ناجح 🎯' : 'لم تتجاوز حد النجاح (50%)'}
            </div>
            <pre className="mt-4 whitespace-pre-wrap text-sm text-zinc-300 leading-7" dir="rtl">
              {result.ai_feedback}
            </pre>
          </motion.div>
        )}
